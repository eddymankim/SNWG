...
import * as THREE from '../acassel/three.js'
...
function FBXLoader(manager=THREE.DefaultLoadingManager) { this.manager = manager }
...
Object.assign(FBXLoader.prototype, {
    ...
    load: function (url, onLoad, onProgress, onError) {
        ...
        loader.load(url, function (buffer) {
            try { onLoad(self.parse(buffer,resourceDirectory)) }
            catch (error) {
                window.setTimeout(function() {
                    if (onError) onError(error)
                    self.manager.itemError(url) }, 0) }
        }, onProgress, onError)
    },
    ...
    parse: function (FBXBuffer, resourceDirectory) {
        ...
        if (isFbxFormatBinary(FBXBuffer)) FBXTree = new BinaryParser().parse(FBXBuffer)
            ...
            if (getFbxVersion(FBXText) < 7000)
                throw new Error('FBXLoader: FBX version not supported.')
            FBXTree = new TextParser().parse(FBXText)
        }

        let connections = parseConnections(FBXTree)
        let images = parseImages(FBXTree)
        let textures = parseTextures(FBXTree,
            new THREE.TextureLoader(this.manager)
                .setPath(resourceDirectory), images, connections)
        ...
        let sceneGraph = parseScene(FBXTree,
            connections, deformers, geometryMap, materials)
        return sceneGraph
    }
...
function parseConnections(FBXTree) {
    ...
    if ('Connections' in FBXTree) {
         ...
         */
        let connectionArray = FBXTree.Connections.properties.connections;
    ...
    parse: function (text) {
        ...
        for (var lineNum = 0, lineLength = split.length; lineNum < lineLength; lineNum ++) {
            ...
            if (match) {
                ...
                // for special case: base64 image data follows
                // "Content: ," line Content: , "iVB..."
