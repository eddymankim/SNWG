...
permalink: /code/cbaumann-workshop-1/
...
///
/// 2017-10-11 Camille Baumann-Jaeger @cbaumann
///

import * as T from '../lib/module.js'

// you should rename this to match your own renderer
import BaumannRenderer from './BaumannRenderer.js'
...
let octahedron = new T.Mesh(
    new T.OctahedronGeometry(10, 1)
    new T.MeshStandardMaterial({ color: 0x900C3F }))
    octahedron.position(-50,0,0)
    octahedron.receiveShadow = true
    octahedron.castShadow = true
    terrain.add(octahedron)
    
let cone = new T.Mesh(
    new T.ConeGeometry(5,10,5,5,true, 0, Math.Pi*2),
    new T.MeshStandardMaterial({color: 0xB30753}))
    cone.position.set(30,0,10)
    box.receiveShadow = true
    box.castShadow = true
    terrain.add(box)
    
let ring = new T.Mesh(
    new T.RingGeometry(1,5,10, 1, 0, Math.PI*2),
    new T.MeshStandardMaterial({ color: 0xC7E78B}))
    ring.position.set(30,0,-40)
...
let renderer = new BaumannRenderer({
