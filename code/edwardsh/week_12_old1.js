...
function moveCamera() {
    if(cameraIsMoving) {
        ...
        remainingSteps -= Math.abs(stepInc);
        
        if(remainingSteps <= 0) {
            player.position.set(otherSideNode.getWorldPosition().x, 
                                otherSideNode.getWorldPosition().y, 
                                otherSideNode.getWorldPosition().z);
            
            // switch active and other end nodes
            var tempEnd = otherEnd;
            otherEnd = activeEnd;
            activeEnd = tempEnd;
...
//            setActiveNode();
            ...
            // rotate player Object back forward relative to the bridge
            var q = new THREE.Quaternion();
            q.setFromAxisAngle(yAxis.normalize(), Math.PI);
            player.applyQuaternion(q);
...
function setActiveNode() {
    for(var i = 0; i < nodes.length; i++) {
        var curNode = nodes[i];
        var dx = Math.pow((player.getWorldPosition().x - curNode.getWorldPosition().x), 2);
        var dy = Math.pow((player.getWorldPosition().y - curNode.getWorldPosition().y), 2);
        var dz = Math.pow((player.getWorldPosition().z - curNode.getWorldPosition().z), 2);
...
function resetOtherNode() {
    for(var i = 0; i < nodes.length; i++) {
        var curNode = nodes[i];
        var dx = Math.pow((otherNode.getWorldPosition().x - curNode.getWorldPosition().x), 2);
        var dy = Math.pow((otherNode.getWorldPosition().y - curNode.getWorldPosition().y), 2);
        var dz = Math.pow((player.getWorldPosition().z - curNode.getWorldPosition().z), 2);
        var dist = Math.sqrt(dx + dy + dz);
