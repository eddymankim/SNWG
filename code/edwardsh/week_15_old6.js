...
var orientMap, orientRef, orientXAxis, orientYAxis, orientZAxis;

var instructUI, uiRead;
...
orientXAxis = orientMap.getObjectByName("xAxis").getWorldDirection().normalize();
orientYAxis = orientMap.getObjectByName("yAxis").getWorldDirection().normalize();
orientZAxis = orientMap.getWorldDirection().normalize();
...
function setScene() {
    ...
    /*
    instructUI = new UI("click = forward\n x = x-axis\n y = y-axis\n z = z-axis", otherSideNode.getWorldPosition());
    scene.add(instructUI);
    */
}
...
// UI object
function UI(message, pos) {
    var loader = new THREE.FontLoader();

    var geo = loader.load( '../../code/edwardsh/fonts/gentilis_regular.typeface.json', function ( font ) {
    ...
    textMesh.position.set(pos.x, pos.y + (cameraHeight * 3), pos.z)
