...
export default class FancyRenderer {
    constructor({
            ...
            gl={
                ...
                useLights:true, useShadows:true, useEffects:true,
                exposure:1.5, whitePoint:1.0,
                tonemapping:M.LinearToneMapping, // Cineon
                shadowType:M.PCFSoftShadowMap, // Uncharted2
                pixelRatio:window.devicePixelRatio, },
            fog={ color:0xCBDBFE, near:1e2, far:1e3 },
            alias={ width:width(), height:height(), },
            bloom={ power:1.0, kernel:36, sigma:1, size:512 },
        ...
        let renderer = new M.WebGLRenderer(gl)
            ...
            renderer.shadowMap.enabled = gl.useLights
            renderer.shadowMap.type = gl.shadowType
            renderer.toneMapping = gl.tonemapping
            renderer.toneMappingExposure = gl.exposure
            renderer.toneMappingWhitePoint = gl.whitePoint
        ...
        let composer = new M.Composer(renderer)
            ...
            if (gl.useEffects) for (let o of effects) composer.add(o)
            // composer.add(new AliasPass(scene,camera,alias))
        ...
        const init = async (e) => {
            // let [skybox] = await importer.load(...files.skybox)
            // this.setEnvMap(skybox)
