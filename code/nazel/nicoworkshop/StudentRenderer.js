...
///
export default class theRenderer {
    ...
    constructor(
      {
        height = 800,         // equals are important - this is not available as parent.
        width = 800,
        background = 0x5A7f8B,
        webGlSettings = {antialias:true },
        fog = {color:0x4ae390, near:1, far: 1000}
        camImputs = {fov:60, aspect: height/width, near: 0.01, far: 10000}
        lightPrime = 0xb385da
      ...
      this.fog = fog;
        ...
        ///
        let renderer = new THREE.WebGLRenderer(webGlSettings);//webGlSettings to keep it smooth
        this.renderer = renderer;
            this.renderer.setPixelRatio(window.devicePixelRatio);
            this.renderer.setSize(this.width, this.height);
            this.renderer.setClearColor(background, 0);  //background, alpha
        ...
        this.scene = scene;
            ...
            this.scene.fog = new THREE.Fog(...Object.vlaues(this.fog)); // ... pull stuff out of array that is values of object fog
            this.scene.background = new THREE.Color(this.background)//creates colors into special format
        ...
        ///
        let camera = new THREE.PerspectiveCamera(...Object.values(camImputs));
        this.camera = camera;
                                                /// 7a. The position of the camera is very important,
                                              ///     so being able to set it from the outside is beneficial.
            this.camera.position.set(1,2,0);  // set() method for many things
        ...
        let sun = new THREE.HemisphereLight(null, null, null)
        ...
        ///
        this.ObjectsToScene = (...sceneElementArray) => for (let element of sceneElementArray) {scene.add(Element) }

        //function addObjectsToScene(...sceneElementArray){
        //  for (let element of sceneElementArray){
        //    scene.add(element);
        //  }
        //}
        ...
        const render = function(){
            requestAnimationFrame(render.bind(this))  /// 10c. First, set up the render function with `requestAnimationFrame`.
            this.controls.update();     ///     10d. Update your camera controller, it has a method called `update`.
          updateFunction(clock.getDelta());  // clock.getDelta returens the time since last frame
                                                  //updateFunction is from constructor
